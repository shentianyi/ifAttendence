      ...
      if request.get?
        ...
        begin
          if params[:batch].nil?
            ...
            unit = Workunit.find_by_id( params[:entityID].strip )
            staff = Staff.find_by_nr( params[:staffID].strip )
            if type == "in"
            ...
            if params[:entityID] && params[:entityID].length>0
              type=params[:logType].strip
              unit = Workunit.find_by_id( params[:entityID].strip )
              Workunit.add_delete_staffs(unit.id,unit.nr,params[:staffIDs],type== "in")
              msg = t("alert.success")
              render :json => { :flag=>true, :msg=>msg, :type=>type}
            else
              render :json => { :flag=>true, :msg=>"未选择需要操作的组员，请刷新页面，重新操作", :type=>type}
            end
          end
        rescue Exception => e
