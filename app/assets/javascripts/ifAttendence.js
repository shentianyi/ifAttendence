$(document).ready(function() {
	...
	$('.allLogout').bind('click', function() {
		var staffNrs = [];
		$(this).parent().prevAll().each(function(index, e) {
			staffNrs.push($(e).attr('staffnr'));
		});
		var t = $(this);
		$.post("./operation", {
			staffIDs : staffNrs,
			entityID : t.parents('ul[unitnr]').attr('unit'),
			logType : "out",
			batch : true
		}, function(data) {
			if (data.flag) {
				var temp = $('h3[unitnr="' + t.parents('ul[unitnr]').attr('unitnr') + '"]').find('label[staffnum]')
				for (var i = 0; i < staffNrs.length; i++) {
					$('li[staffnr=' + staffNrs[i] + ']').remove();
					temp.text(parseInt(temp.text()) - 1);
				}
			}
		}, 'json');
...
function batch_login() {
	...
	if ($('#worku-title-show').attr("unit")) {
		var staffs = {};
		$('.staff-choosen').each(function(index, e) {
			staffs[$(e).children('label[memnr]').text()] = $(e).children().first().text();
		});
		var staffNrs = Object.keys(staffs);
		$.post("./operation", {
			staffIDs : staffNrs,
			entityID : $('#worku-title-show').attr("unit"),
			logType : $('#log').attr("state"),
			...
			if (data.flag) {
				var workunitNr = $('#worku-title-show').text();
				var temp = $('h3[unitnr="' + workunitNr + '"]').find('label[staffnum]')
				if (data.type == 'in') {
					for (var i = 0; i < staffNrs.length; i++) {
						$('ul[unitnr="' + workunitNr + '"]').prepend($('<li staffnr=' + staffNrs[i] + ' >' + staffNrs[i] + ' : ' + staffs[staffNrs[i]] + '</li>'));
						temp.text(parseInt(temp.text()) + 1);
					}
				} else {
					for (var i = 0; i < staffNrs.length; i++) {
